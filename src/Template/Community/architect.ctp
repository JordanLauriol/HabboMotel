<p class="title is-4 is-uppercase"><?= __('Les architectes l\'hôtel'); ?></p>

<?php
foreach($architect as $rank => $players) {
    echo '<div class="column team">
            <p class="title is-4">' . $rank . '</p>
         ';

    foreach($players as $player) {
        echo '<div class="user" style="background: url(../img/background-architect.png) no-repeat;" data-uid="' . $player->id . '">
